...
int main(int argc, char* argv[]) {
    ...
    //Mod monitor pentru a trimite comenzile prin semnale
        if (argc == 2 && strcmp(argv[1], "monitor") == 0) {
            struct sigaction sa1, sa_term;
  
            //Functia apelata cand se primeste un semnal 
            void handle(int sig) {
        ...
        printf("[MONITOR] Terminating with delay...\n");
        fflush(stdout);
        usleep(2000000); //asteapta 2 secunde inainte de terminare
        exit(0);
        }
        
        sa1.sa_handler = handle;
        ...
        printf("[MONITOR] Waiting for commands...\n");
        fflush(stdout);
        while (1) pause();
        }
        //Mod clasic, care executa comenzile direct 
            if (argc < 2) {
            usage(argv[0]);
            return 1;
        }
